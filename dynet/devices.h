...
namespace Eigen {
  ...
  struct GpuDevice;
  // CudaStreamDevice not found, using GpuStreamDevice instead
  typedef GpuStreamDevice CudaStreamDevice;
}
...
class Device {
 ...
 public:
   ...
   virtual void reset_rng(unsigned /*seed*/) {
       std::cerr << "[dynet] WARNING: reset_rng called, but it does not do anything." << std::endl;
   };
  int device_id;
  DeviceType type;
  MemAllocator* mem;
  float* kSCALAR_MINUS_ONE{};
  float* kSCALAR_ONE{};
  float* kSCALAR_ZERO{};
  std::string name;
...
class Device_GPU : public Device {
 public:
  ...
  ~Device_GPU() noexcept(false) override;
  void reset_rng(unsigned seed) override;
  int cuda_device_id;
...
inline void show_pool_mem_info() {
  ...
  for (Device* dev : devs) {
    std::cout << " Device " << dev->name << " - FOR Memory " << (dev->pools[0]->get_cap() >> 20)
